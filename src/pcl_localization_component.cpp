...
: rclcpp::Node("pcl_localization", options),
  clock_(RCL_ROS_TIME),
  ...
  broadcaster_(this)
{
  ...
  RCLCPP_INFO(get_logger(), "Initializing PCL Localization Node");
  ...
  RCLCPP_INFO(get_logger(), "PCL Localization Node Initialized");
}
...
void PCLLocalization::initializePubSub()
{
  ...
  score_pub_ = create_publisher<pcl_localization_ros2::msg::LocalizationScore>(
...
void PCLLocalization::cloudReceived(const sensor_msgs::msg::PointCloud2::ConstSharedPtr msg)
{
  ...
  auto score_msg = pcl_localization_ros2::msg::LocalizationScore();
  score_msg.header.stamp = msg->header.stamp;
  score_msg.header.frame_id = global_frame_id_;
  score_msg.has_converged = has_converged;
  score_msg.fitness_score = fitness_score;
  score_msg.transformation_probability = registration_->getTransformationProbability();
  score_msg.final_num_iteration = registration_->getFinalNumIteration();
  score_msg.align_time = align_time;
  ...
  // Fill transformation matrix in score message
  for (int i = 0; i < 4; i++) {
    for (int j = 0; j < 4; j++) {
      score_msg.final_transformation[i * 4 + j] = final_transformation(i, j);
    }
  }
  score_pub_->publish(score_msg);
